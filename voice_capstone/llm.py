...
1. primary_concern (main farm concern, string)
2. duration (how long issue present, string like "3 days" or "2 weeks")
3. severity (severity level 1-10, integer)
4. progression (one of: "improving", "worsening", "stable")
5. associated_issues (list of additional issues, array of strings)
6. affected_crop (crop type or field area, string)
7. onset_type (one of: "sudden", "gradual")
8. environmental_factors (weather, water, soil conditions, string)
9. farm_management_history (farm practices, recent treatments, or previous instances, string)
...
"""
    ...
    symptom_text_lines = []
    if symptom_record.get('primary_concern'):
        symptom_text_lines.append(f"Primary Concern: {symptom_record['primary_concern']}")
    ...
    fallback_lines = []
    if symptom_record.get('primary_concern'):
        fallback_lines.append(f"Primary Concern: {symptom_record['primary_concern']}")
    if symptom_record.get('duration'):
...
The user has a low-to-moderate farm issue. Provide concise, practical farm advice (e.g., irrigation checks, nutrient strategy category, scouting cadence, sanitation, and resistant variety planning).

Guidelines:
- Keep it to 1-2 short, friendly sentences.
- Do NOT claim certainty.
- If suggesting fertilizers, use category-level recommendations (e.g., balanced NPK, nitrogen-heavy, potassium support, micronutrient blend) and advise soil-test confirmation.
- Frame it as practical field guidance rather than guaranteed treatment.

Return ONLY the advice text.
"""
    
    symptom_text = f"Primary Concern: {symptom_record.get('primary_concern', 'crop issue')}"
    
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": f"Please provide practical farm tips for this crop issue:\n{symptom_text}"}
    ]
    ...
    # Generic fallback
    return "Start with field scouting and moisture checks, then use a balanced nutrient plan and validate exact fertilizer choice with a local soil test."


def generate_farm_advice(crop_record: Dict[str, Any], session_id: str) -> str:
    """Agriculture alias used by intake/main flows."""
    return generate_health_advice(crop_record, session_id)


def respond_to_consult(user_question: str, symptom_record: Optional[Dict[str, Any]] = None, conversation_history: list = None) -> str:
    """
    Answer user's general agriculture questions safely and practically.
    ...
    context_str = ""
    if symptom_record:
        context_str = f"\n**Previous Session Context (For Reference Only):**\nThe user recently completed a crop assessment with these details:\n{json.dumps(symptom_record, indent=2)}\nUse this context if relevant, but prioritize answering their current question."

    system_prompt = f"""You are a versatile and professional Farm Consult Assistant.

{LLM_SAFETY_INSTRUCTIONS}

Capabilities:
- **Crop & Soil Guidance**: Explain crop issues, soil and nutrient basics, and likely causes.
- **Actionable Recommendations**: Suggest near-appropriate interventions, including fertilizer type categories and IPM options.
- **Pest/Disease Management**: Provide practical prevention and control strategies.
- **Agronomy Clarification**: Explain agricultural terms and trade-offs.

Guidelines:
- **EXTREMELY CONCISE**: Keep your answers short and to the point. Use bullet points for readability. Avoid long paragraphs.
- **ACTIONABLE**: Give the best-fit recommendation first, then 1-2 alternatives.
- **FERTILIZER REQUESTS**: Provide likely suitable fertilizer categories (e.g., balanced NPK, higher-N, potassium-rich, micronutrient foliar), with a short reason.
- **UNCERTAINTY HANDLING**: If crop/stage/soil is missing, ask one concise follow-up OR provide a conditional recommendation.
- **EMPATHETIC**: Maintain a supportive and professional tone.{context_str}
- **HIGH-RISK FIELD ALERTS**: If severe spread/crop loss risk appears, advise urgent local agronomist/extension follow-up.
...
"""
    ...
    return response.strip() if response else "I couldn't process that right now. For urgent crop risk, contact a local agronomist while I retry."
